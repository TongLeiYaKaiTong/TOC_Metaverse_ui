    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:sysCategory:stop']"
              type="success"
          ...
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:sysCategory:remove']"
              type="danger"
              icon="el-icon-delete"
              size="mini"
              :disabled="multiple"
        ...
        >
          ...
          <el-table-column label="排序" width="75" prop="sequence" align="center" sortable="custom" />
          <el-table-column
            label="装扮分类"
            width="200"
            prop="name"
          ...
          <el-table-column label="图标" align="center">
            <template slot-scope="scope">
              <el-image :src="scope.row.icon" style="width: 50px; height: 50px" fit="contain" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="管理" width="160" fix="right" class-name="small-padding fixed-width" align="center">
            <template slot-scope="scope">
              <el-button
                v-permisaction="['admin:sysCategory:preview']"
                size="mini"
                type="text"
                icon="el-icon-edit"
      ...
      <!-- 分类上传弹框 -->
      <el-dialog :title="dialog.title" :visible.sync="dialog.open" width="400px">
        <el-form ref="form" :model="form" :rules="dialog.rules" label-width="auto">
          <el-row>
            ...
            <el-col :span="22">
              <el-form-item label="分类排序" prop="sequence">
                <el-input v-model="form.sequence" placeholder="请输入分类排序" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
...
import {
  ...
  statusCategoryApi,
  deleteCategoryApi
} from '@/api/admin/sys-decoratecate'
      ...
      dialog: {
        ...
        rules: {
          ...
          name: [
            ...
            { max: 20, message: '装扮名称：长度限制20汉字', trigger: 'blur' }
          ],
          sequence: [{ required: true, message: '分类排序不能为空', trigger: 'blur' }]
        }
      },
      // 表单参数
      form: {
  ...
  methods: {
    ...
    handleStatus(row, status) {
      ...
      this.confirmFun(
        ...
        () => {
          return statusCategoryApi({ ids, status })
        },
        (response) => {
    ...
    submit() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          let submitApi
          if (this.form.id) {
            submitApi = updateCategoryApi
          } else {
            if (!this.form.file) {
              this.msgError('请选择要上传的文件')
              return
            }
            submitApi = uploadCategoryApi
          }

          // 创建FormData 文件传输必须的
          const formData = new FormData()
          this.form.id && formData.append('id', this.form.id) // 要提交给后台的文件,并且字段的key为id
          this.form.file && formData.append('file', this.form.file) // 要提交给后台的文件,并且字段的key为file
          formData.append('index', this.form.index) // 要提交给后台的文件,并且字段的key为index
          formData.append('name', this.form.name) // 要提交给后台的文件,并且字段的key为title
          formData.append('sequence', this.form.sequence) // 要提交给后台的文件,并且字段的key为singer
          // 上传
    ...
    edit(row) {
      ...
      // this.form.fileList = [{ url: row.icon }]
    },
    // 分类上传重置
    reset() {
      this.form = {
    ...
    getList() {
      this.loading = true
      listCategoryApi(this.queryParams).then((response) => {
